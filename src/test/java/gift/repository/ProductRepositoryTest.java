...
class ProductRepositoryTest {
    ...
    class saveTests {
        ...
        void 널_값이_포함된_상품_삽입_시_예외_발생() {
            ...
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(allNullProduct));

            Product nameNullProduct = new Product(null, null, 5000, "http://kakao.com", true, false);
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(nameNullProduct));

            Product priceNullProduct = new Product(null, "Test Product", null, "http://kakao.com", true, false);
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(priceNullProduct));

            Product imageUrlNullProduct = new Product(null, "Test Product", 5000, null, true, false);
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(imageUrlNullProduct));

            Product validatedNullProduct = new Product(null, "Test Product", 5000, "http://kakao.com", null, false);
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(validatedNullProduct));

            // 명시적 deleted=null 사용 시 nullable 위반. 생성자에서 deleted 비명시 시 자동 false로 설정됨
            Product deletedNullProduct = new Product(null, "Test Product", 5000, "http://kakao.com", true, null);
            assertThrows(DataIntegrityViolationException.class, () -> productRepository.save(deletedNullProduct));
        }
    }
    ...
    class findByIdTests {
        ...
        void 존재하는_상품의_ID로_조회_시_상품반환() {
            assertThat(productRepository.findByIdAndDeletedIsFalse(existingProduct.getId())).isPresent();
        }
        ...
        void 삭제된_상품의_ID로_조회_시_빈Optional반환() {
            ...
            assertThat(productRepository.findByIdAndDeletedIsFalse(existingProduct.getId())).isEmpty();
        }
